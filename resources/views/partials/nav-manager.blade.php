    ...
    <ul class="navbar-nav ml-auto">
      ...
      <li class="nav-item dropdown">
        <a class="nav-link" data-toggle="dropdown" href="#">
          ...
          <span class="badge badge-warning navbar-badge">@if($notificationsNumber > 0) {{ $notificationsNumber }} @endif</span>
        </a>
        <div class="dropdown-menu dropdown-menu-lg dropdown-menu-right">
          <span class="dropdown-item dropdown-header">Notifications</span>
          <div class="dropdown-divider"></div>

          @if(count(\App\Models\Order::where('payment_status', true)->whereDate('created_at' , \Carbon\Carbon::now()->subHours(3))->get()) > 0)
          <a href="{{ url("manager/orders") }}" class="dropdown-item">
            <i class="fas fa-shopping-cart mr-2"></i>{{ count(\App\Models\Order::where('payment_status', true)->whereDate('created_at' , \Carbon\Carbon::now()->subHours(3))->get()) }} commande(s)
            <span class="float-right text-muted text-sm">Récemment</span>
          </a>
          @endif
          
          <div class="dropdown-divider"></div>
          <a href="{{ url("manager/users") }}" class="dropdown-item">
          ...
          <div class="dropdown-divider"></div>
          <a href="{{ url("activity") }}" class="dropdown-item dropdown-footer">Voir toutes les notifications</a>
        </div>
      </li>
      <li class="nav-item">
