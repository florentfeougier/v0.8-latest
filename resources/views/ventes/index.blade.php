...
<section class="instagram p-t-0">
  <div class="container animated bounceInUp">
        ...
        @foreach($ventes as $sale)
        <div class="block4 wrap-pic-w p-1 animated slideInUp">
          <div class="block4-square" style="position:relative;width:100%;height:400px; line-height:400px; background:{{$sale->color_code}};">
            <h3 class="text-center my-auto" style="line-height:400px;" >{{$sale->name}}</h3>
          </div>
          <a href="{{url("ventes/$sale->slug")}}" class="block4-overlay sizefull ab-t-l trans-0-4">
            <span class="block4-overlay-heart  text-white  flex-m trans-0-4 p-4">
              ...
              <span class="p-t-2 ">
                ...
                {{ date('d/m/Y', strtotime($sale->date)) }} de {{$sale->horaires}}</span>
            </span>

            <div class="block4-overlay-txt trans-0-4 p-4">
              <h3 class="text-white">{{$sale->name}}</h3>
              <p class="s-text10 m-b-15 h-size1 of-hidden">
                {{substr($sale->description, 0, 180)}}...
              </p>
...
@if($sale->show_location == true)
   {{ $sale->location_address }} - {{$sale->location_postalcode}}
@else
  Lieu non dévoilé
@endif
              </span>
      ...
      <div class="py-3">
        ...
        @foreach(\App\Models\Vente::where('status', 'closed')->take(5)->get() as $vente)
