...
@section('template_title')
    {{$product->name}} - Vente de {{$vente->name}}
@endsection
...
<!-- Product Detail -->
    ...
    <!-- breadcrumb -->
    	<div class="bread-crumb bgwhite flex-w py-4 animated bounceInLeft">
    		...
    		<a href="{{url("ventes/$vente->slug")}}" class="s-text16">
    			{{$vente->name}}
          <span class=" mx-2">></span>
    		</a>

    		<a href="{{url("ventes/$vente->slug#products")}}" class="s-text16">
    			Produits
          <span class=" mx-2">></span>
    		</a>
		...
		<div class="flex-w flex-sb ">
            ...
            <img width="60px" class="product-thumbnail" data-type='image' data-content="{{asset("$product->picture_two")}}" src="{{asset("$product->picture_two")}}" alt="{{$product->name}}" title="{{$product->name}}">
          @endif
          @if(!is_null($product->video))

            <img width="60px" class="product-thumbnail" data-type='video' data-content="{{asset("$product->video")}}" height="60px"src="{{asset("storage/images/play-button.png")}}" alt="{{$product->name}}">
@endif
	...
	<!-- Relate Product -->
		<section class=" bgwhite py-4">
          ...
          <h6 class="fontsize-5 mt-2 t-center font-3">Une sélection de plantes et accéssoires</h6>
	          ...
	          @foreach($vente->products->take(8) as $relatedProduct)
	                    ...
	                    <a href="{{url("fiches-entretien/$relatedProduct->slug")}}" class="text-center bg4 bo-rad-23 hov1 text-white btn">
	                      Fiche d'entretien
								...
								<div class="block2-txt mt-2">
									<a href="{{url("fiches-entretien/$relatedProduct->slug")}}"  class="badge fontsize-1 block2-name dis-block ">
										{{$relatedProduct->name}}
