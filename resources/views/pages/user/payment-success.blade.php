...
@if($order->payment_status == true)
  ...
  Votre commande a été payé le {{$order->update}} avec comme référence de paiement {{$order->payment_id}}
...
@if($order->cart == "shop")
  passé sur le shop
@else
  pour la vente de {{ \App\Models\Vente::where('slug', $order->cart)->first()->name }} du {{ \App\Models\Vente::where('slug', $order->cart)->first()->date }}
@endif
</p>


@if($order->payment_status == true)
  <span class="badge badge-info">Référence de paiement: {{$order->payment_id}}</span>
  ...
  <tbody>
    ...
    <tr>
      <th scope="row"> <a href="{{url("profile/" . Auth::user()->name . "/commandes/" . $product->order_id )}}">
        <img src="{{asset("storage/$product->thumbnail")}}" height="100" alt="">
      </a> </th>
      <td>{{$product->name}}</td>
