...
<a class="h5 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="{{url("profile/" . Auth::user()->name . '/paiements')}}" >Paiements</a>
...
<div class="row">
<div class="col-lg-12">
<h1 class=" text-white">VOS PAIEMENTS ({{count(Auth::user()->payments)}})</h1>
...
<a href="{{url('profile/' . Auth::user()->name . '/edit')}}" class="btn btn-secondary">Modifier mes infos de facturation</a>
...
<a href="{{url('contact')}}" class="btn btn-sm btn-secondary">Contacter le support</a>
</div>
</div>
</div>
<div class="card-body">
...
<div class="table-responsive">
  ...
  <tbody>
    ...
    @foreach($user->payments as $payments)
    <tr>
      <th scope="row"> <a href="{{url("profile/" . Auth::user()->name . "/paiements/" . $payments->payments_id )}}">{{$payments->payments_id}}</a> </th>
      <td>{{$payments->created_at}}</td>
      <td>{{$payments->amount}}€</td>
      <td>
        @if($payments->status == "payed")
          <a href="{{url('')}}" class="badge badge-pill badge-success">Validé</a> </td>

        @else
          <a href="{{url("compte/paiements/$payments->payments_id")}}" class="badge badge-pill badge-warning">Payer maintenant</a> </td>
