...
@section('content')
  ...
  {{-- @if( count(\App\Models\Order::where('payment_status', false)->get()) > 0)
...
<a class="h5 mb-0 text-white text-uppercase d-none d-lg-inline-block" href="{{url("profile/" . Auth::user()->name . '/commandes')}}" >Commandes</a>
...
<div class="row">
<div class="col-lg-12">
<h1 class=" text-white">VOS COMMANDES ({{count(Auth::user()->orders)}})</h1>
...
<a href="{{url('profile/' . Auth::user()->name . '/plantes')}}" class="btn btn-success">Voir vos plantes</a>
    ...
    <tr>
      ...
      <td>
        @if($order->payment_status == true && $order->payment_id != null)
          <a href="{{url("profile/" . Auth::user()->name . "/commandes/" . $order->payment_id)}}" class="badge badge-pill badge-success">Validé</a> </td>

        @else
          <a href="{{url("profile/" . Auth::user()->name . "/commandes/" . $order->order_id)}}" class="badge badge-pill badge-warning">Payer maintenant</a> </td>
